...
class Gui(Tk):
    ...
    def animation_button_clicked(self, event):
        time = self.choices_selected.get()
        if (self.state==0)and (time == "Weekly"):
            self.state = 1
            self.animation_button.configure(text="Stop Animation")
            self.animation_image.configure(image=self.weekly_image,
                                           height=20,
                                           width=20)
            self.tick()
            
        elif (self.state==0) and (time == "Monthly"):
            self.state = 1
            self.animation_button.configure(text="Stop Animation")
            self.animation_image.configure(image=self.monthly_image,
                                           height=20,
                                           width=20)
            self.tick()
            
        elif (self.state==0) and (time == "Yearly"):
            self.state = 1
            self.animation_button.configure(text="Stop Animation")
            self.animation_image.configure(image=self.yearly_image,
                                           height=20,
                                           width=20)
            self.tick()
    ...
    def tick(self):
        ...
        self.animation_image.place(x=self.image_x_pos, 
                                    y=self.image_y_pos)
        if self.image_x_pos >= 350:
            self.image_x_change = -2
        elif self.image_y_pos >= 180:
            self.image_y_change = -1
    ...
    def add_animation_image(self):
        self.animation_image = Label(self.animation_frame)
    ...
    def add_animation_frame(self):
        self.animation_frame = Frame(self.global_frame,height="200",width="370")
        self.animation_frame.grid(row=7, column=0)
        self.animation_frame.configure(background="green")
